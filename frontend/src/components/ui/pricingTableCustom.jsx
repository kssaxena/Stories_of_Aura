...
const BASE_PACKAGES = [
  {
    ...
    defaults: { creatives: 8, stories: 6, reels: 4 },
  },
  {
    id: "growth",
    name: "Growth Base",
    description: "For scaling brands",
    defaults: { creatives: 14, stories: 12, reels: 8 },
  },
  {
    id: "enterprise",
    name: "Enterprise Base",
    description: "For high-volume brands",
    defaults: { creatives: 20, stories: 30, reels: 12 },
  },
];
...
const CustomPackageModal = ({ onClose }) => {
  ...
  // ✅ FIX 1: stabilize entries for mobile
  const deliverables = useMemo(() => Object.entries(values), [values]);
  ...
  return (
    <div className="w-full h-fit max-w-6xl rounded-3xl p-8 shadow-xl z-50">
      {/* Header */}
      <h2 className="text-3xl font-bold mb-2">Build Your Custom Package</h2>
      <p className="mb-8">
        Select and start with a base and customize everything to match your
        goals.
      </p>
      ...
      {base && (
        <>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {deliverables.map(([key, val]) => (
              <div key={key} className="border rounded-xl p-4">
                <p className="font-semibold capitalize">{key}</p>
                <input
                  type="number"
                  inputMode="numeric" // ✅ FIX 3: mobile keyboard stability
                  min={0}
                  value={val}
                  onChange={(e) =>
                    setValues({
      ...
      <div className="mt-10 flex justify-center gap-10 ">
        ...
        <Button Label="Request Custom Package" type="submit" />
      </div>
    </div>
  );
};
